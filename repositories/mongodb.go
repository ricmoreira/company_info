...
func NewDBCollections(config *config.Config) *DBCollections {
	...
	headersIndex := mongo.IndexModel{
		Keys:    keys,
		Options: options,
	}

	headerCollection := db.Collection("headers")
	_, err = headerCollection.Indexes().CreateOne(context.Background(), headersIndex)
	if err != nil {
		log.Fatal(err)
	}
